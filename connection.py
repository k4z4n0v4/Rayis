# ProtonVPN with polybar ipc hooks added.
# Copy to /usr/lib/python3.x/site-packages/protonvpn_cli/connection.py
...
def disconnect(passed=False):
    ...
    if is_connected():
        ...
        if is_connected():
            ...
            if not passed:
                print("Disconnected.")
                os.system("polybar-msg hook vpn 3")
    else:
        if not passed:
    ...
    with open(os.path.join(CONFIG_DIR, "ovpn.log"), "r") as f:
        while True:
            ...
            if "Initialization Sequence Completed" in content:
                ...
                os.system("polybar-msg hook vpn 2")
                break
